...
class Install extends Company
{
    ...
    public function install() {
        ...
        $this->addColumn('province', 'varchar');
    ...
    public function inputCategoryData() {
        ...
        $this->setCategory("티비 TV", "", "internet.png");
    ...
    public function updateProvinceCity()
    {
        $companies = $this->loadQuery("region != ''");
        if ( $companies ) {
            foreach( $companies as $com ) {
                $region = $com->region;
                $city = null;
                $province = null;
                if ( strpos($region, ' - ') ) {
                    list($province, $city) = explode(' - ', $region, 2);
