...
class Entity {
    ...
    /**
     *
     * 쿼리를 통해서 Entity 의 id 값만 로드한다.
     *
     * 즉, 레코드의 전체 내용을 로드하는 것이 아니라, id 만 로드하므로 로드하는 속도가 빠르다.
     *
     * id 만 먼저 로드해서 그 중 몇 id 만 로드하고자 하는 경우에 유용하게 사용 할 수 있다.
     *
     * 예를 들어, 특정 쿼리에 맞는 id 를 여러개 추출해서 그 중 하나만 로드하고자 하는 경우...
     *
     * @note 쿼리를 통한 Entity 를 로드하는데, 결과에는 Entity id 를 배열로 리턴한다.
     *
     * @param null $where
     * @param string $fields
     * @return array
